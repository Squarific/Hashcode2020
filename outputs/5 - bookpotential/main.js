...
function start () {
  var libraryId = getLibraryWithHighestBookPotential();
  var currentDay = 0;
  ...
  while (libraryId != null) {
    ...
    libraryId = getLibraryWithHighestBookPotential();
  }
...
function getLibraryWithHighestBookPotential (timeRemaining) {
  ...
  for (var k = 0; k < libraries.length; k++) {
    ...
    var bookPotential = Math.min(libraries[k].shipCapacity * timeRemaining, libraries[k].length);
    
    if (mostBookPotential < bookPotential) {
      mostId = k;
      mostBookPotential = bookPotential;
    }
  }  
